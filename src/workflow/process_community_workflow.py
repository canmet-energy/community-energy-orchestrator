...
def duplicate_missing_timeseries(timeseries_dir, building_type, required_count):
    ...
    files = sorted(
        ...
        if f.startswith(building_type) and f.endswith("-results_timeseries.csv")
    )
    if not files:
...
def copy_archetype_files(community_name, requirements):
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(copy_single_archetype, src_file, dst_file): (src_file, dst_file)
            for src_file, dst_file in copy_tasks
        }
...
def update_weather_location(community_name):
    ...
    print(
        f"[PARALLEL] Updating weather location in {len(h2k_files)} files with {max_workers} workers"
    )

    updated_count = 0
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(update_single_weather_file, h2k_file, weather_location): h2k_file
...
def collect_timeseries_parallel(output_dir, timeseries_dir):
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(_copy_single_timeseries, bdir, timeseries_dir): bdir
...
def run_hpxml_conversion(community_name, requirements):
    ...
    if output_dir.exists():
        ...
        if collected < completed_sims:
            ...
            print(
                f"[WARNING] {missing_count} simulations completed but did not produce timeseries output"
            )
    else:
        collected = 0
    ...
    for building_type, required_count in requirements.items():
        actual_count = duplicate_missing_timeseries(
            timeseries_dir_path, building_type, required_count
        )
        if actual_count < required_count:
            print(
                f"[ERROR] Failed to reach required count for {building_type}: {actual_count}/{required_count}"
            )
...
def main(community_name):
    ...
    if cleanup_dir.exists() and cleanup_dir.is_dir():
        ...
        try:
            ...
            print(
                f"[ERROR] Some files may be open in an editor. Close all files in this community folder and try again."
            )
            raise PermissionError(
                f"Permission denied when cleaning {community_name} directory. Close any open files and try again."
            ) from e
        except ValueError:
            raise ValueError(
                f"Safety check failed: {cleanup_dir} is not within communities directory"
            )
