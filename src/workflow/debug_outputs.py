...
def _validate_single_h2k(h2k_file, community_name):
    ...
    try:
        ...
        if not contents.strip().startswith("<?xml") and not contents.strip().startswith(
            "<HouseFile"
        ):
        ...
        if not is_valid:
            return (
                h2k_file,
                f"Location Code: {location_code}\n  Validation: FAILED (does not match expected code for {community_name})",
                location_code,
            )
...
def debug_weather_h2k(community_name):
    ...
    print(
        f"[PARALLEL] Validating weather location in {len(h2k_files)} H2K files with {max_workers} workers"
    )

    validation_results = []
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(_validate_single_h2k, h2k_file, community_name): h2k_file
